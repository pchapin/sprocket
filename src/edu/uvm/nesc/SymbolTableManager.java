...
/**
 * Class that manages a stack of symbols. This allows proper handling of nested scopes. Whenever
 * a new scope is entered, a new symbol table is created for that scope. Right now the only
 * symbol information managed by this class is that symbol's status as a type name. Properly
 * parsing nesC requires that identifiers that are type names be tokenized as such. This class
...
public final class SymbolTableManager {
            ...
            // throws SymbolTable.SymbolRedefinitionException, SymbolTable.SymbolConflictException
    {
        try {
            ...
            // know how to allow user defined exceptions through ANTLR generated code. Google
            // for "ANTLR throws spec" (the ANTLR site is down at the moment).
        }
    }


    /**
     * Adds an ordinary identifier (non-type name) to the currently active scope (at the top of
     * the scope stack).
    ...
    /**
     * Pops the scope stack by removing the top symbol table. This method should be called
