...
/**
 * This super type provides a dictionary of name to ID pairs. Each such dictionary is taken from the specified file. The
 * meaning of the names and ID values is not significant to this class. Each instance of this class defines its own
 * namespace; ID values stored in one instance are not related to ID values stored in another. This class is a thin
 * wrapper around Scala's built in Map collection. However it does provide some added functionality (such as reading
 * the dictionary file) so creating a separate class for this concept is reasonable.
 *
 * There is no way to add (name, ID) pairs after the constructor of this class executes. Thus all names "known" to this
 * class must necessarily be in the provided dictionary file.
 *
 * Note that this class is abstract. Use one of the two subclasses defined below.
 */
abstract class EntityMap(fileName: File) {

  protected val idMap = Map[String, Int]()
  ...
  /**
   * Check to see if the given named entity is known to this map. "Known" names are those explicitly specified in the
   * configuration file used to initialize the EntityMap object. such names have an associated ID and information object.
   *
   * @param name The name being checked.
  ...
  /**
   * Display all the (name, ID) paris in this EntityMap. The result is displayed on the standard output device. The
   * output is indented to allow for a header created elsewhere.
   */
  def dumpIds() {
...
/**
 * A specialized EntityMap for holding (interface name, interface ID) mappings. A separate class is defined to avoid
 * accidentally mixing up different map types.
 */
class InterfaceMap(fileName: File) extends EntityMap(fileName) {
  ...
  /**
   * Display information about all the interfaces known to this map onto the standard output device. The displayed
   * information is indented so that a header can be provided elsewhere.
   */
  def dumpInterfaces() {
     for (name <- idMap.keysIterator) {
...
/**
 * A specialized EntityMap for holding (component name, component ID) mappings. A separate class is defined to avoid
 * accidentally mixing up different map types.
 */
...
/**
 * A specialized EntityMap for holding (role name, role ID) mappings. A separate class is defined to avoid accidentally
 * mixing up different map types.
 */
